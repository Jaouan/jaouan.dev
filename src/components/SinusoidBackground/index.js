...
class SinusoidBackground extends HTMLElement {
    connectedCallback() {
        ...
        svgScrollRevealElement.setAttribute("data-speed", this.getAttribute('data-speed') || 1);
        svgScrollRevealElement.setAttribute("data-y-offset", this.getAttribute('data-y-offset') || 0);
        ...
        this.observer = new ResizeObserverDebounced({
            resizingCallback: (entries) => entries.forEach(_ => this.classList.remove("ready")),
            resizedCallback: (entries) => entries.forEach(entry => {
                const height = entry.contentRect.height;
                if (!this.lastHeight || Math.abs(this.lastHeight / height - 1) > CHANGE_THRESHOLD) {
                    this.lastHeight = height;
                    svgScrollRevealElement.innerHTML = "";
                    svgScrollRevealElement.appendChild(generateVerticalSinusoidSVG(height, sinWidth, sinHeight));
